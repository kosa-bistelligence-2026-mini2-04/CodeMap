...
class TestConfigFallback(unittest.TestCase):
    ...
    def test_settings_initialization_without_env_file(self):
        ...
        # 시스템 환경 변수를 빈 딕셔너리로 마스킹하여 순수 코드 레벨 기본값으로 기동을 테스트합니다.
        with patch.dict(os.environ, {}, clear=True):
            settings = Settings(_env_file=None)
            
            # 1. DB 상세 접속 정보 폴백 검증
            self.assertEqual(settings.DB_USER, "postgres")
            ...
            # 3. DATABASE_URL 동적 조립 결과 검증
            expected_db_url = "postgresql+asyncpg://postgres:@localhost:5432/codemap_db"
            self.assertEqual(settings.DATABASE_URL, expected_db_url)
