...
## 파일 내용 최대 허용 문자 수 (초과 시 truncated=True 반환)
...
def _detect_language(file_path: str) -> str | None:
    '''파일 경로의 확장자로 언어 식별자를 반환한다. 미등록 시 None.'''
    name = Path(file_path).name.lower()
    ## Dockerfile 같은 확장자 없는 특수 파일 처리
...
def _read_file_safe(clone_root: Path, rel_path: str) -> tuple[str, bool]:
    '''
    클론 루트 기준 상대 경로 파일을 안전하게 읽어 (내용, 잘림여부) 튜플을 반환한다.

    path traversal 재확인 후 utf-8 → cp949 → latin-1 순으로 인코딩을 시도하며,
    모두 실패하면 utf-8 errors=replace 로 강제 읽는다.
    '''
...
# ──────────────────────────────────────────────
# API-FILE: 분석 저장소 파일 내용 조회
# GET /api/repo/analysis/{job_id}/files/content
# ──────────────────────────────────────────────
@router.get(
    "/api/repo/analysis/{job_id}/files/content",
    response_model=FileContentResponse,
    summary="분석 저장소 파일 내용 조회",
    description="clone된 저장소 내 특정 파일의 텍스트 내용을 반환한다.",
    responses={
        403: {"model": ErrorResponse, "description": "허용되지 않는 파일 경로 (path traversal)"},
        404: {"model": ErrorResponse, "description": "workspace 미준비 또는 파일 없음"},
        422: {"model": ErrorResponse, "description": "바이너리 파일"},
    },
)
async def get_file_content(
    job_id: UUID,
    path: str = Query(..., description="저장소 내 상대 파일 경로"),
    db: AsyncSession = Depends(get_db),
) -> FileContentResponse:
    '''
    분석 저장소 clone 디렉토리에서 파일 내용을 읽어 반환한다.

    - path traversal 이중 방어 (세그먼트 검사 + resolve().relative_to())
    - 바이너리 확장자는 422 반환
    - 50,000자 초과 시 truncated=True 로 잘라서 반환
    - 블로킹 파일 I/O는 asyncio.to_thread 로 격리
    '''
    service = AnalysisService(db)
    ...
    if not target.exists() or not target.is_file():
        raise WorkspaceNotReadyError(message=f"파일을 찾을 수 없습니다: {clean_path}")
