...
- 질문의 성격에 따라 다음 기준에 맞게 Tool과 scope를 선택하십시오:
  * `search` : 특정 기능, 개념, 비즈니스 로직에 대한 의미론적 검색 (권장 scope: "chunk")
  * `dir` : 프로젝트 전체 아키텍처, 폴더/파일 트리 구조, 모듈 구성을 파악할 때 반드시 사용 (전체 구조 파악 시 path는 ".", 권장 scope: "directory")
  * `grep` : 특정 클래스명, 함수명, 변수명이 쓰인 정확한 위치(라인)를 찾을 때 사용 (권장 scope: "file")
  * `read` : 분석에 필수적인 핵심 파일(README.md, package.json 등)의 전체 내용을 읽을 때 사용 (권장 scope: "file")
