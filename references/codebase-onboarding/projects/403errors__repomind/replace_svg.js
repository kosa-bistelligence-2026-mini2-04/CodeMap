...
const path = '/Users/gojo/Documents/GitHub/repomind/src/lib/prompt-builder.ts';
let content = fs.readFileSync(path, 'utf8');

const regex = /              - Example 1 \(Linear Pipeline\):[\s\S]*?<\/svg>\n```/g;
const match = content.match(regex);

if (!match) {
  console.error('Could not find the SVG block');
  process.exit(1);
}

const replacement = `              - Example 1 (Linear Pipeline):
\`\`\`svg
...
console.log('Successfully replaced SVGs');
