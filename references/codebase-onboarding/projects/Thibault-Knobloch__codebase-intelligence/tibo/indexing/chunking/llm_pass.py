...
from ...utils import FILE_CHUNKS_FILE_PATH, OPENAI_API_KEY, OPENAI_API_URL, LOCAL_LLM, LOCAL_MODEL_NAME, LOCAL_LLM_URL, save_json
...
def process_json_file(project_description: str, project_structure: str) -> None:
    ...
    llm_model = f"LOCAL {LOCAL_MODEL_NAME}" if LOCAL_LLM == "True" else "OPENAI gpt-4o-mini"
    click.secho(f"Processing {total_chunks} chunks across {total_files} files using {llm_model}...")
    with click.progressbar(length=total_files, label="", show_pos=True) as bar:
        for file_path, chunks in data.items():
            ...
            if code_chunks:  # Only make a call if there are unsummarized chunks
                if LOCAL_LLM == "True":
                    summaries = generate_local_summaries_for_file(code_chunks, file_path, project_description, project_structure)
                else:
                    summaries = generate_openai_summaries_for_file(code_chunks, file_path, project_description, project_structure)
...
def generate_openai_summaries_for_file(
...
def generate_local_summaries_for_file(
    code_chunks: List[str],
    file_path: str,
    project_description: str,
    project_structure: str
    ...
    """
    Generate minimal summaries for all code chunks in a file using a local LLM.
    """
    retry_delays = [1, 2, 4, 8, 16]  # Exponential backoff delays in seconds
    if not any(chunk.strip() for chunk in code_chunks):
        return ["Empty chunk: No functionality to summarize."] * len(code_chunks)
