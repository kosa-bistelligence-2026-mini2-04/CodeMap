...
export const translations = {
  en: {
    ...
    hero: {
      ...
      folderSelected: "{count} files ready · {skipped} generated or sensitive files excluded",
      localBridgeNotice: "CodeMap uploads the selected source files to an isolated temporary workspace. Dependencies, build output, Git history, environment files, oversized files, and unsafe paths are excluded.",
      localAnalyze: "Upload folder and start analysis",
  ...
  ko: {
    ...
    hero: {
      ...
      folderSelected: "분석할 파일 {count}개 · 생성물·민감 파일 {skipped}개 제외",
      localBridgeNotice: "선택한 소스 파일은 격리된 임시 작업 공간으로 업로드됩니다. 의존성 폴더, 빌드 결과, Git 이력, 환경설정 파일, 대용량 파일, 안전하지 않은 경로는 자동으로 제외합니다.",
      localAnalyze: "폴더 업로드 후 분석 시작",
