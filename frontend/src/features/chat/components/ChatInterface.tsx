...
export function ChatInterface({
  ...
  const [prevProps, setPrevProps] = useState({ repoId, threadId });

  // 컴포넌트 렌더링 중에 repoId나 threadId 변경을 감지하고 상태를 즉시 초기화하여
  // 하단의 localStorage 저장 useEffect가 이전 repo/thread의 메시지를 새 repo에 저장(누수)하는 것을 원천 차단합니다.
  if (repoId !== prevProps.repoId || threadId !== prevProps.threadId) {
    setPrevProps({ repoId, threadId });
    setMessages([]);
