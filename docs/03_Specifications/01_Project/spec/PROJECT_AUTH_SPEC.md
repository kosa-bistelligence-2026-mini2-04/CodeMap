...
- Refresh Token: 만료 시간 7일, DB에 저장
- 응답: `{ code: 200, message: "success", data: { accessToken, refreshToken, expiresIn } }`
...
- `app/common/auth.py`에 `get_current_user(token: str = Depends(oauth2_scheme))` 구현
...
| 필드명 | 타입 | 필수 | 설명 |
| --- | --- | --- | --- |
| refreshToken | String | Y | 기존 발급된 Refresh Token |
...
`backend/.env` 및 `core/config.py`에 아래 항목 추가 필요:
