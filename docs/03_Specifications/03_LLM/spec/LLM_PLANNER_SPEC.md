...
`LLM-PLANNER`(Supervisor)는 사용자 질문을 분석해 오타 교정·의도 보정한 쿼리(`rewritten_query`)와 탐색 도구 실행 계획(`access_plan`)을 도출하는 **LLM 계획 수립 에이전트**입니다. 로컬 I/O 도구는 직접 보유하지 않습니다(보안 원칙).
...
### 3. 완료 조건
- 폴백 시에도 `route_node` → 워커 fan-out이 정상 동작하도록 유효한 `access_plan`을 반환한다.
...
- **현황**: 현재 `supervisor_node`는 `user_query`만 읽는 단발 계획 수립이며, 피드백 입력 경로는 미구현.
