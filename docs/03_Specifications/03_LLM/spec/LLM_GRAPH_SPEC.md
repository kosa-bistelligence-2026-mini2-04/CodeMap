...
> **도메인**: LLM | **모듈**: LLM-GRAPH | **최종 업데이트**: 2026-06-23

## 범위

`LLM-GRAPH`는 LangGraph 기반 데이터 수집 계층입니다. 사용자-facing HTTP/SSE 처리는 `chat/`에서 담당하고, 이 계층은 StateGraph 실행과 `CodeMapState` 갱신에 집중합니다.
...
| 기능 ID | 기능명 | 계층 | Phase |
| --- | --- | --- | --- |
| LLM-GRAPH-B-201 | CodeMapState 스키마 정의 | Backend | Phase 1 |
| LLM-GRAPH-B-202 | LangGraph workflow 정의 | Backend | Phase 1 |

---

## LLM-GRAPH-B-201: CodeMapState 스키마 정의

| 항목 | 내용 |
| --- | --- |
| 분류 | Backend |
| 모듈명 | GRAPH |
| 구현 위치 | `backend/app/agent/state.py` |
...
## LLM-GRAPH-B-202: LangGraph workflow 정의

| 항목 | 내용 |
| --- | --- |
| 분류 | Backend |
| 모듈명 | GRAPH |
| 구현 위치 | `backend/app/agent/graph.py` |
