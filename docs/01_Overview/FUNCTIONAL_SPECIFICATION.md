...
> [!NOTE]
> 아래 `chat/` 및 `agent/` 구조는 최신 합의안을 기준으로 한 **구현 예정/권장 구조**입니다. 현재 파일이 모두 존재한다는 뜻이 아니라, 이후 agent 기능 구현 시 맞춰갈 목표 구조입니다.
> `Final Answer Agent`는 LangGraph 데이터 수집 그래프 내부가 아니라 `chat/` Application Layer에 둡니다.

```plain text
CodeMap/
├── frontend/                 # Next.js 16 + React 19 (FSD & Bulletproof 패턴)
│   ├── src/
│   │   ├── app/              # 📄 [라우팅 영역] Next.js App Router (page.tsx, layout.tsx)
│   │   ├── common/           # 🌐 [공통 영역] 버튼, 모달, 유틸, 훅 등 순수 재사용 요소
│   │   └── features/         # 🧠 [도메인 영역] 도메인 특화 컴포넌트, 훅, API 통신 로직
│   ├── next.config.ts        # Next.js 환경 설정 및 API Rewrite 프록시 설정
│   └── package.json
...
### 3. API 및 통신 설계
* 로컬 개발 환경에서 Frontend(포트 3000)와 Backend(포트 8000)는 `mkcert`를 통한 **HTTPS 기반 CORS 통신 및 API Rewrite 프록시**를 수행합니다.
...
LangGraph
  ...
  4. Evidence Aggregator Node
     ...
     - compact_context 생성
↓
LangGraph returns CodeMapState
↓
chat/service

  5. Final Answer Agent LLM (Application Layer)
     - State 원본 근거 기반 최종 답변 생성
     - chat/service를 통해 SSE 스트리밍
     - 파일 위치: chat/final_answer_agent.py
↓
User
```
...
| `LLM-CHAT-B-101` | LLM | CHAT | Backend | 멀티에이전트 채팅 실행 API | `POST /api/chat/{repo_id}/runs` 요청을 받아 chat run을 생성하고 LangGraph 실행을 시작 |
| `LLM-CHAT-B-201` | LLM | CHAT | Backend | chat/service 실행 관리자 | 세션 관리, LangGraph 비동기 실행 호출, run 상태 저장, SSE stream URL 반환 |
| `LLM-CHAT-B-202` | LLM | CHAT | Backend | Final Answer Agent | LangGraph가 반환한 State의 원본 근거와 compact_context를 기반으로 최종 답변 생성 |
...
| 기능 ID | 카테고리 | 도메인(모듈) | 구분 | 기능명 | 상세 설명 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `PROJECT-LIST-B-202` | PROJECT |  | Backend | Service | 프로젝트 목록 조회 및 관리 |
| `PROJECT-LIST-B-301` | PROJECT |  | Backend | Repository | 분석 job metadata 저장 |
...
| `PROJECT-PIPELINE-F-201` | PROJECT |  | Frontend | UI Component | 현재 분석 수준 안내 메시지 |
| `PROJECT-PIPELINE-F-202` | PROJECT |  | Frontend | UI Component | 얕은/깊은 분석 분리 프로그레스 UI |
| `PROJECT-PIPELINE-F-301` | PROJECT |  | Frontend | API/Query | 진행률 실시간 수신 |
| `PROJECT-REPO-B-303` | PROJECT |  | Backend | Repository | 중복 저장소 검사 |
| `RAG-GRAPH-B-201` | RAG |  | Backend | Service | 의존성 그래프 시각화 |
